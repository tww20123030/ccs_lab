...
void http_serve(int fd, const char *name)
{
    ...
    strcat(pn, name);/* pn may be overwritten by name */
    split_path(pn);/*get pathname and filename */
...
void url_decode(char *dst, const char *src)
{
    for (;;)
    {
        ...
        else/* the dst can also be overwritten if there are too many % or + in src */
        {
            *dst = *src;
            src++;
